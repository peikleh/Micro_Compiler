...
_lr_productions = [
  ...
  ('read_stmt -> READ L_PAR id_list R_PAR SEMI','read_stmt',5,'p_read_stmt','parser.py',117),
  ('write_stmt -> WRITE L_PAR id_list R_PAR SEMI','write_stmt',5,'p_write_stmt','parser.py',120),
  ('return_stmt -> RETURN expr SEMI','return_stmt',3,'p_return_stmt','parser.py',123),
  ('expr -> expr_prefix factor','expr',2,'p_expr','parser.py',126),
  ('expr_prefix -> expr_prefix factor addop','expr_prefix',3,'p_expr_prefix','parser.py',129),
  ('expr_prefix -> empty','expr_prefix',1,'p_expr_prefix','parser.py',130),
  ('factor -> factor_prefix postfix_expr','factor',2,'p_factor','parser.py',134),
  ('factor_prefix -> factor_prefix postfix_expr mulop','factor_prefix',3,'p_factor_prefix','parser.py',138),
  ('factor_prefix -> empty','factor_prefix',1,'p_factor_prefix','parser.py',139),
  ('postfix_expr -> primary','postfix_expr',1,'p_postfix_expr','parser.py',143),
  ('postfix_expr -> call_expr','postfix_expr',1,'p_postfix_expr','parser.py',144),
  ('call_expr -> id L_PAR expr_list R_PAR','call_expr',4,'p_call_expr','parser.py',148),
  ('expr_list -> expr expr_list_tail','expr_list',2,'p_expr_list','parser.py',151),
  ('expr_list -> empty','expr_list',1,'p_expr_list','parser.py',152),
  ('expr_list_tail -> COMM expr expr_list_tail','expr_list_tail',3,'p_expr_list_tail','parser.py',155),
  ('expr_list_tail -> empty','expr_list_tail',1,'p_expr_list_tail','parser.py',156),
  ('primary -> L_PAR expr R_PAR','primary',3,'p_primary','parser.py',159),
  ('primary -> id','primary',1,'p_primary','parser.py',160),
  ('primary -> INTLITERAL','primary',1,'p_primary','parser.py',161),
  ('primary -> FLOATLITERAL','primary',1,'p_primary','parser.py',162),
  ('addop -> PLUS','addop',1,'p_addop','parser.py',168),
  ('addop -> MINUS','addop',1,'p_addop','parser.py',169),
  ('mulop -> MULT','mulop',1,'p_mulop','parser.py',173),
  ('mulop -> DIV','mulop',1,'p_mulop','parser.py',174),
  ('if_stmt -> s_if L_PAR cond R_PAR decl stmt_list else_part ENDIF','if_stmt',8,'p_if_stmt','parser.py',178),
  ('s_if -> IF','s_if',1,'p_s_if','parser.py',183),
  ('else_part -> s_else decl stmt_list','else_part',3,'p_else_part','parser.py',187),
  ('else_part -> empty','else_part',1,'p_else_part','parser.py',188),
  ('s_else -> ELSE','s_else',1,'p_s_else','parser.py',193),
  ('cond -> expr compop expr','cond',3,'p_cond','parser.py',197),
  ('compop -> LESS','compop',1,'p_compop','parser.py',200),
  ('compop -> MORE','compop',1,'p_compop','parser.py',201),
  ('compop -> EQ','compop',1,'p_compop','parser.py',202),
  ('compop -> N_EQ','compop',1,'p_compop','parser.py',203),
  ('compop -> L_EQ','compop',1,'p_compop','parser.py',204),
  ('compop -> R_EQ','compop',1,'p_compop','parser.py',205),
  ('while_stmt -> s_while L_PAR cond R_PAR decl stmt_list ENDWHILE','while_stmt',7,'p_while_stmt','parser.py',208),
  ('s_while -> WHILE','s_while',1,'p_s_while','parser.py',211),
